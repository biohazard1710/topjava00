...
public class ExceptionInfoHandler {
        ...
        if (rootMsg != null) {
            ...
            for (Map.Entry<String, String> entry : CONSTRAINS_I18N_MAP.entrySet()) {
                if (lowerCaseMsg.contains(entry.getKey())) {
                    message = messageSource.getMessage(entry.getValue(), null, req.getLocale());
                }
            }
        }
    ...
    public ErrorInfo handleMethodArgumentNotValidException(HttpServletRequest req, MethodArgumentNotValidException e) {
        return logAndGetErrorInfo(req, e, false, VALIDATION_ERROR, ValidationUtil.getFieldErrors(e.getBindingResult()));
    }
    ...
    private static ErrorInfo logAndGetErrorInfo(HttpServletRequest req, Exception e, boolean logException, ErrorType errorType, String message) {
        ...
        return new ErrorInfo(req.getRequestURL(), errorType, message != null ? message : rootCause.toString());
    }
